"""Prepare a reviewed legacy runtime registry candidate from explicit approvals.

This command intentionally writes a candidate registry only.  It never edits
the checked-in registry and always performs a fresh child-process replay when
...
from knowledge_platform.baseline.legacy_runtime_review_queue import load_legacy_runtime_review_queue
from scripts.phase0a_legacy_runtime_review_queue import _replay
...
def _load_queue(path: Path) -> dict[str, Any]:
    try:
        ...
        raise ValueError("legacy runtime review queue is invalid") from exc
    return load_legacy_runtime_review_queue(raw)
...
def prepare_reviewed_registry(
    *,
    ...
    if replay_for_validation["status"] != "matched":
        raise ValueError("legacy runtime replay is not matched")
    ...
    for raw_probe in document["executed_probes"]:
        ...
        # Preserve the exact queue decision in the candidate so a later gate
        # run can audit which evidence item authorized this status transition.
        review["approval_review_id"] = approved_by_probe[probe_id]
        review["approval_source"] = "explicit-local-review-queue"
    reviewed = validate_legacy_runtime_probe_registry(
        document,
        repo_root=repo_root,
        require_reviewed=True,
        replay_evidence=replay_for_validation,
    )
    if not reviewed.reviewed or validation.reviewed:
    ...
    return {
        "status": "PHASE0A_LEGACY_RUNTIME_BOUNDARY_REVIEW_CANDIDATE_READY_NOT_FROZEN",
...
def main() -> int:
    ...
    parser.add_argument("--review-queue", type=Path, default=Path("artifacts/phase0a/legacy-runtime-boundary-review-queue.json"))
    ...
    try:
        ...
        print(json.dumps({"format": "agent-knowledge-platform-legacy-runtime-boundary-review-candidate/v1", "error": str(exc)}))
        return 1
