"""Generate a same-checkout, non-releaseable Phase 10 dependency SBOM shadow."""
...
from knowledge_platform.distribution.dependency_sbom import build_dependency_sbom_shadow

_ROOT = Path(__file__).resolve().parents[2]
_DEFAULT_OUTPUT = _ROOT / "artifacts/phase0b-local-catalog/phase10-dependency-sbom-shadow.json"
...
def run_shadow(*, repo_root: Path = _ROOT, output_path: Path = _DEFAULT_OUTPUT) -> dict[str, object]:
    result = build_dependency_sbom_shadow(repo_root=repo_root, source_revision=_source_revision(repo_root))
        ...
        json.dumps(
            {
                ...
                "python_component_count": result["python_component_count"],
                "node_component_count": result["node_component_count"],
