...
def load_combined_workspace(root: Path | str, manifest: dict, *, _initializing_ok: bool = False) -> dict:
    ...
    return {"schema_space_id": wiki["space_id"], "evidence_root": root / "wiki-evidence", "schema_bundle": wiki["schema_bundle"], "catalog": root / "catalog.sqlite3", "file_bindings": {**documents["file_bindings"], **wiki["file_bindings"]},
            "document_bindings": documents["document_bindings"], "document_resources":documents.get("document_resources"), "space_ids": sorted(set(documents["space_ids"]) | set(wiki["space_ids"])),
