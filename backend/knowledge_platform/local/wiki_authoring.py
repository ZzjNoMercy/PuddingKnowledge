...
class WikiAuthoringStore:
    def __init__(self, *, database: Path, space_id: str, bundle, raw_hashes, raw_manifest_sha256):
    ...
    def from_owned_workspace(cls, owned):
        ...
        store=cls(database=Path(owned['catalog']),space_id=source.space_id,bundle=source.bundle,
                  raw_hashes=source.raw,raw_manifest_sha256=source.raw_manifest_sha256)
    ...
    def _initialize(self, db, *, pages, index, log):
        ...
        db.execute('CREATE TABLE IF NOT EXISTS knowledge_wiki_authoring_state (space_id TEXT PRIMARY KEY, revision TEXT NOT NULL, index_text TEXT NOT NULL, log_text TEXT NOT NULL)')
        ...
        db.execute('INSERT INTO knowledge_wiki_authoring_state VALUES (?,?,?,?)',(self.space_id,revision,index,log))
