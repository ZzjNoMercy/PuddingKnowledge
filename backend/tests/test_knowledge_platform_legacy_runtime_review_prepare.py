...
from scripts.phase0a_legacy_runtime_review_prepare import prepare_reviewed_registry
from scripts.phase0a_legacy_runtime_review_queue import build_review_queue
from tests.test_knowledge_platform_legacy_runtime_review_queue import _replay_evidence
...
def _paths(root: Path, tmp_path: Path) -> tuple[Path, Path, dict[str, object]]:
    ...
    return root / "docs/knowledge-platform/phase-0a-legacy-runtime-call-probes.yaml", queue_path, queue
...
def test_prepare_requires_all_explicit_review_ids_and_writes_candidate_only(tmp_path: Path) -> None:
    ...
    candidate = (tmp_path / "candidate.yaml").read_text(encoding="utf-8")
    assert "status: reviewed" in candidate
    assert "approval_source: explicit-local-review-queue" in candidate
    assert registry_path.read_text(encoding="utf-8") != candidate
