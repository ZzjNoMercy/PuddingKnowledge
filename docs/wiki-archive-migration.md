...
The port is internal and is not automatically enabled at startup. Multi-page model orchestration, Catalog/query/read projection of its active pages, public administration integration and lifecycle recovery commands remain unfinished. Do not enable this port for normal product use until those integrations are complete. No production activation or complete repository separation claim follows from these transaction tests.
