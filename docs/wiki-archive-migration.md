...
## Persistent generation admission and explicit local Worker

`POST /v1/wiki/authoring/enqueue` accepts `{request: GENERATE_REQUEST, not_before: UNIX_SECONDS}`. The complete immutable request, actor and due timestamp are receipt-bound before success. `not_before` is an integer UTC Unix timestamp, zero for immediately eligible; boolean/negative/out-of-range values reject. Admission validates current revision and selected owned evidence but does not invoke the model. Exact enqueue replay is idempotent; changed requests/due times with the same ID reject. There is a 50000-admission per-Space budget; pruning/retention administration is not yet implemented.

`POST /v1/wiki/authoring/queue` accepts `{space_id, after?, limit?}` and returns operation/status/receipt metadata, with keyset pagination (default 20, maximum 50). It verifies actual proposal records and bounds aggregate verification bytes. Queue state mirrors proposal `unsettled/ready/failed/abandoned`; absent a proposal, it is `queued` or terminal `rejected`. This read never dispatches work. Same Space admin permissions apply. Saved scopes never grant Worker authority: dispatch uses the current principal and only its own saved actor's admissions.

`POST /v1/wiki/authoring/run_queue` with `{space_id}` processes at most one due admission. For automatic local consumption, start the API with `--wiki-authoring --wiki-authoring-model-config /absolute/model.json --wiki-authoring-worker`. The explicit Worker checks for due admissions once per second inside the same owned-Home process. Without the Worker flag, no background consumption occurs. It generates proposals only; publication still requires apply. Queue responses expose whether this actual Worker thread is running and a generic fault code; proposal execution liveness remains unknown.

Execution authority remains the existing durable proposal claim. If a process dies before claiming, the admission can safely be reconsidered; after claim, an unsettled proposal is never retried. The claim transaction checks queue actor/request/state/due time, so direct generate cannot bypass a rejected or future admission using that ID. A stale revision or failed pre-claim validation becomes terminal rejected; no automatic rebase or source substitution occurs. Corrupt queue/proposal evidence stops the background Worker rather than making up a success. Unknown live attempts can be explicitly abandoned through the recovery action.

Shutdown requests Worker stop and waits at most one second; an in-flight remote call may leave an unsettled record when the process exits. This does not claim that inference was cancelled. New attempts need new explicit operation IDs. Cancellation and rescheduling are described below. Admission retention, broader multi-user dispatch authority, Console management and whole-installation production scheduling/cutover/rollback remain open.

## Cancel or reschedule an unclaimed admission

`POST /v1/wiki/authoring/control_queue` requires `space_id`, `operation_id`, `command_id`, `expected_receipt`, `reason`, and `action` (`cancel` or `reschedule`). Only reschedule also takes `not_before`. Use the latest `admission_receipt` from queue inspection, or `result_receipt` from the preceding control result. Reasons are nonempty and limited to 4096 UTF-8 bytes. Control requires admin, exact Space scope and the original admission actor.

Only a queued admission with no proposal can be controlled. Cancellation is terminal and blocks Worker and direct generation claims under that operation ID. Rescheduling changes only due time, preserving generation request/revision/Raw selection. Both controls run under the same SQLite write transaction used by proposal claiming: if the claim wins, control rejects; if control wins, the Worker must obey the new state/time before claiming. For an already-unsettled proposal, use abandon instead.

The current queue receipt binds a verified history head. Events bind actor, command/reason, generation-request digest, prior head/due/receipt, resulting due/state and timestamp. State and event commit together, with at most 128 controls per admission. Exact command retries return that command's original result, even after later controls; inspect queue for current state. Altering a reused command ID rejects. Original enqueue retries still compare against the original admitted due time, not a later schedule.

Older queue rows receive an empty history column and retain their existing receipt. Controlled rows verify the complete bounded chain; missing, extra or modified events reject. A later automatic pre-claim rejection retains the history head and binds its rejected state in a new current receipt; it does not invent an operator control event. Control does not rewrite or remove historical records. Retention, richer multi-user dispatch and Console management remain open.
