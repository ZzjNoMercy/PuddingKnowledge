...
Shutdown requests Worker stop and waits at most one second; an in-flight remote call may leave an unsettled record when the process exits. This does not claim that inference was cancelled. New attempts need new explicit operation IDs. Queue cancellation/rescheduling, admission retention, broader multi-user dispatch authority, Console management and whole-installation production scheduling/cutover/rollback remain open.
